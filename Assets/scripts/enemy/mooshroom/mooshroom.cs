...
public class mooshroom : EnemyBase
{
    public enum EnemyState { Idle, Roaming, FollowingPlayer, Attacking, TrackingLastSeen }
    public EnemyState currentState;
    public player players; // Reference to the player for interaction
    ...
    public int scoreValue = 10; // Score value for killing the enemy
    ...
    {   
        ...
        if (players == null || rb == null) return;
    ...
    {   
        ...
        {
            roamTarget = ((Vector2)players.transform.position - (Vector2)transform.position).normalized;
            AvoidObstacles(ref roamTarget);
    ...
    void HandleAttackingState(float distanceToPlayer)
    {
        ...
        {
            AttackPlayer();
            animator.SetFloat("relpos" , players.transform.position.x - transform.position.x);
    ...
    void AttackPlayer()
    {
        ...
        {
            audioManager.PlaySFX(audioManager.mooshroomSFX);
    ...
    void UpdatePlayerPath()
    {
        if (currentState == EnemyState.FollowingPlayer || currentState == EnemyState.Attacking)
        {
            ...
            playerPath.Enqueue(players.transform.position);
        }
    }
